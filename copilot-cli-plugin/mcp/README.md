...
For unattended hosts (CI, containers, AKS, VMs) you can flip a lever to use an
Azure **Managed Identity** instead, so the tools run with no interactive
`az login`:
...
```json
{
  "mcpServers": {
    "chaos-studio": {
      "command": "chaos-mcp",
      "env": {
        "CHAOS_MCP_AUTH_MODE": "managed-identity",
        "CHAOS_MCP_MSI_CLIENT_ID": "<user-assigned-identity-client-id>"
      }
    }
  }
}
